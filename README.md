...
- Ticketmaster Discovery returns price *ranges* per event; SeatGeek returns event-level
  stats (lowest price + listing count). Neither official public API exposes individual
  seat rows, so alerts key off the lowest available public price per source.
